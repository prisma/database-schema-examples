create sequence service_provider_performance_summary_type_id_seq
;

create sequence provider_performance_prisma_id_seq
;

create sequence service_performance_prisma_id_seq
;

create sequence service_provider_performance_prisma_id_seq
;

create sequence service_provider_performance_summary_prisma_id_seq
;
...
alter table provider_performance
		add constraint provider_performance_providers_npi_fk
				foreign key (npi) references providers
;

create table service_provider_performance
(
		hcpcs_code text not null,
		npi integer not null
...
create index svcs_hcpcs_drug_indicator_index
		on services (hcpcs_drug_indicator)
;

alter table provider_performance
		add constraint provider_performance_cms_svcs_hcpcs_code_fk
				foreign key (hcpcs_code) references services
;

alter table service_provider_performance
...
create table providers_organizations
(
		npi integer not null
...
create index service_performance_rank_max_avg_submitted_charge_amt_in
		on service_performance (rank_max_avg_submitted_charge_amt)
;
...
create index service_performance_rank_min_avg_mcare_standardized_amou
		on service_performance (rank_min_avg_mcare_standardized_amt)
;

create index service_performance_rank_max_avg_mcare_standardized_amou
		on service_performance (rank_max_avg_mcare_standardized_amt)
;

create index service_performance_rank_var_avg_mcare_standardized
		on service_performance (rank_var_avg_mcare_standardized_amt)
;
...
create index service_performance_rank_est_ttl_submitted_charge_amt_inde
		on service_performance (rank_est_ttl_submitted_charge_amt)
;

create index service_performance_rank_est_ttl_mcare_allowed_amt_inde
		on service_performance (rank_est_ttl_mcare_allowed_amt)
;

create index service_performance_rank_est_ttl_mcare_standardized_amt
		on service_performance (rank_est_ttl_mcare_standardized_amt)
;

create index service_performance_rank_var_avg_mcare_submitted_ch
		on service_performance (rank_var_avg_mcare_submitted_charge_pay_amt)
;

create table service_provider_performance_summary
(
		npi integer not null
		...
		rank_var_est_ttl_mcare_submitted_charge_pay_amount_even_longer_than_before_more_length_yo bigint,
...
create unique index service_provider_performance_summary_prisma_id_uindex
		on service_provider_performance_summary (prisma_id)
;

create index service_provider_performance_summary_entity_type_index
		on service_provider_performance_summary (entity_type)
;

create index prvdr_smry_rank_ttl_hcpcs_code_index
		on service_provider_performance_summary (rank_ttl_hcpcs_code)
;

create index prvdr_smry_rank_ttl_n_of_svcs_index
		on service_provider_performance_summary (rank_ttl_n_of_svcs)
;

create index prvdr_smry_rank_est_ttl_submitted_charge_amt_index
		on service_provider_performance_summary (rank_est_ttl_submitted_charge_amt)
;

create index prvdr_smry_rank_est_ttl_mcare_pay_amt_index
		on service_provider_performance_summary (rank_est_ttl_mcare_pay_amt)
;

create index prvdr_smry_rank_var_est_ttl_mcare_submitted_charge_pa
		on service_provider_performance_summary (rank_var_est_ttl_mcare_submitted_charge_pay_amoun)
;

create index prvdr_smry_rank_est_ttl_mcare_pay_amt_by_ttl_hcpc
		on service_provider_performance_summary (rank_est_ttl_mcare_pay_amt_by_ttl_hcpcs_code)
;

create index prvdr_smry_rank_est_ttl_mcare_pay_amt_by_ttl_numb
		on service_provider_performance_summary (rank_est_ttl_mcare_pay_amt_by_ttl_n_of_servi)
;

create table service_provider_performance_summary_type
(
		id serial not null
...
create unique index service_provider_performance_summary_type_slug_uindex
		on service_provider_performance_summary_type (slug)
;

alter table service_provider_performance_summary
		add constraint service_provider_performance_summary_service_provider_performan
				foreign key (summary_type) references service_provider_performance_summary_type
;

CREATE VIEW live_service_performance AS SELECT performance.hcpcs_code,
		...
		FROM (provider_performance performance
			JOIN providers providers ON ((providers.npi = performance.npi)))
	GROUP BY performance.hcpcs_code, providers.entity_type
;

CREATE VIEW live_service_provider_performance AS SELECT performance.hcpcs_code,
		performance.npi,
		providers.entity_type,
...
CREATE VIEW live_service_provider_performance_summary_drug_no AS SELECT 3 AS summary_type,
		performance.npi,
		...
		FROM (service_provider_performance performance
			JOIN services svcs ON ((svcs.hcpcs_code = performance.hcpcs_code)))
	WHERE (svcs.hcpcs_drug_indicator = 'N'::text)
	GROUP BY performance.npi, performance.entity_type
;

CREATE VIEW live_service_provider_performance_summary_drug_yes AS SELECT 2 AS summary_type,
		performance.npi,
		performance.entity_type,
		count(performance.hcpcs_code) AS ttl_hcpcs_code,
		sum(performance.n_of_svcs) AS ttl_n_of_svcs,
...
CREATE VIEW live_service_provider_performance_summary_overall AS SELECT 1 AS summary_type,
		performance.npi,
		performance.entity_type,
		count(performance.hcpcs_code) AS ttl_hcpcs_code,
		sum(performance.n_of_svcs) AS ttl_n_of_svcs,
