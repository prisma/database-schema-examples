...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER after_insert_following AFTER INSERT
ON followings FOR EACH ROW
BEGIN
  IF NEW.followed_type = 'Club' THEN
    IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = NEW.followed_id AND user_id = NEW.user_id) THEN
      UPDATE user_club_activities
      SET following = true
      WHERE user_id = NEW.user_id AND club_id = NEW.followed_id;
    ELSE
      INSERT INTO
        user_club_activities (user_id, club_id, following)
      VALUES
        (NEW.user_id, NEW.followed_id, true);
    END IF;
  END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER after_delete_following AFTER DELETE
ON followings FOR EACH ROW
BEGIN
  IF OLD.followed_type = 'Club' THEN
    IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = OLD.followed_id AND user_id = OLD.user_id) THEN
      UPDATE user_club_activities
      SET following = false
      WHERE user_id = OLD.user_id AND club_id = OLD.followed_id;
    ELSE
      INSERT INTO
        user_club_activities (user_id, club_id, following)
      VALUES
        (OLD.user_id, OLD.followed_id, false);
    END IF;
  END IF;
END */;;
DELIMITER ;
...
CREATE TABLE `logins` (
  ...
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `index_logins_on_ip` (`ip`(4)) USING BTREE,
  KEY `index_logins_on_logged_in_at` (`logged_in_at`) USING BTREE,
  KEY `index_logins_on_user_id` (`user_id`) USING BTREE,
  KEY `index_logins_on_country_id` (`country_id`) USING BTREE,
  KEY `index_logins_on_device_type` (`device_type`(191)) USING BTREE
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER after_insert_place AFTER INSERT
ON places FOR EACH ROW
BEGIN
  IF NEW.user_id THEN
    IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = NEW.placable_id AND user_id = NEW.user_id) THEN
      UPDATE user_club_activities
      SET played = 1, wishing = false
      WHERE user_id = NEW.user_id AND club_id = NEW.placable_id;
    ELSE
      INSERT INTO
        user_club_activities (user_id, club_id, checked_in, played, wishing)
      VALUES
        (NEW.user_id, NEW.placable_id, false, 1, false);
    END IF;
  END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `insertPost` BEFORE INSERT ON `posts` FOR EACH ROW SET NEW.publish = IFNULL(NEW.publish, UTC_TIMESTAMP()) */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
/*!50003 SET character_set_results = @saved_cs_results */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER club_score_update_after_insert_review AFTER INSERT
ON reviews FOR EACH ROW
BEGIN
    UPDATE clubs
    SET score = (SELECT AVG(reviews.score) / 2 FROM reviews WHERE reviews.club_id = clubs.id AND reviews.deleted_at = '1970-01-01 00:00:00')
    WHERE id = NEW.club_id;

    IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = NEW.club_id AND user_id = NEW.user_id) THEN
      UPDATE user_club_activities
      SET review_score = NEW.score, played = 1
      WHERE user_id = NEW.user_id AND club_id = NEW.club_id;
    ELSE
      INSERT INTO
        user_club_activities (user_id, club_id, review_score, played)
      VALUES
        (NEW.user_id, NEW.club_id, NEW.score, 1);
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER club_score_update_after_update_review AFTER UPDATE
ON reviews FOR EACH ROW
BEGIN
    UPDATE clubs
    SET score = (SELECT AVG(reviews.score) / 2 FROM reviews WHERE reviews.club_id = clubs.id AND reviews.deleted_at = '1970-01-01 00:00:00')
    WHERE id = OLD.club_id OR id = NEW.club_id;

    IF NEW.deleted_at = '1970-01-01 00:00:00' THEN
      IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = NEW.club_id AND user_id = NEW.user_id) THEN
        UPDATE user_club_activities
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER club_score_update_after_delete_review AFTER DELETE
ON reviews FOR EACH ROW
BEGIN
    UPDATE clubs
    SET score = (SELECT AVG(reviews.score) / 2 FROM reviews WHERE reviews.club_id = clubs.id AND reviews.deleted_at = '1970-01-01 00:00:00')
    ...
    IF EXISTS (SELECT id FROM user_club_activities WHERE club_id = OLD.club_id AND user_id = OLD.user_id) THEN
      UPDATE user_club_activities
      SET review_score = NULL
...
CREATE TABLE `schema_migrations` (
  ...
  PRIMARY KEY `unique_schema_migrations` (`version`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
